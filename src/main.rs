...
fn main() -> anyhow::Result<()> {
    ...
    // Load the hard-coded credentials into the store
    {
        ...
        let mut init_settings = AppSettings::new();
        init_settings.ap_ssid = Some(app_config.wifi_ssid.to_string());
        init_settings.ap_pass = Some(app_config.wifi_pass.to_string());
        init_settings.dexcom_user = Some(app_config.dexcom_user.to_string());
        init_settings.dexcom_pass = Some(app_config.dexcom_pass.to_string());

        tx_channel.send(init_settings).unwrap();
        store.check_updates();
    }
